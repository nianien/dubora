"""
Subtitle Model: 字幕系统的唯一事实源（SSOT）v1.3
...
v1.3 升级：
- speaker 提升为对象（SpeakerInfo），包含 id、gender、speech_rate、emotion
- utterance 顶层不再散落 speech_rate、emotion，统一收编到 speaker 对象内
- gender 信息从 SSOT 自然流向下游（align → dub_manifest → TTS 性别兜底）
...
class SpeakerInfo:
    ...
    speech_rate: SpeechRate = field(default_factory=lambda: SpeechRate(zh_tps=0.0))
...
class SubtitleUtterance:
    ...
    - speaker: 说话人信息（SpeakerInfo 对象，包含 id、gender、speech_rate、emotion）
    - start_ms: 开始时间（毫秒）
    - end_ms: 结束时间（毫秒）
    ...
    - speaker.speech_rate.zh_tps 必须存在
    - utterances 不允许时间重叠

    注意：
    - v1.3：speaker 提升为 SpeakerInfo 对象，speech_rate 和 emotion 收编到 speaker 内
    - 这是 Subtitle Model v1.3 中的 Utterance，与 schema/types.py 中的 Utterance（ASR 原始响应）不同
...
class SubtitleModel:
    ...
    极简 SSOT 设计：
    - 最小字段集：只保留会被下游用到的
    - 结构明确：speaker 对象内聚说话人属性，cue 层只包含原始字幕事实
    - 不把 raw/additions 塞进模型（那是 ASR 原始事实，不是 SSOT）
    - SSOT 只保存原始事实，不包含任何翻译或执行信息
    ...
    - utterances: 连续说话单元列表（speaker 包含语速、情绪、性别信息）

    各阶段职责（ownership 清晰）：
    - asr_post：写 utterances、cues[*].source、start/end/speaker(含 gender/speech_rate/emotion)
    - mt：不写 SSOT（翻译结果单独保存）
    - tts：不写 SSOT（只读生成 tts_jobs）
    """
